...
export async function generateTryOnWithNanoBanana({
  ...
  try {
    ...
    if (!candidates || candidates.length === 0) {
      // Trim large base64 data for logging
      ...
      const errorMsg = `No image generated by Gemini (No candidates). Response: ${JSON.stringify(logData)}`;
    ...
    if (!imagePart) {
      ...
      errorMsg += ` Full Response: ${JSON.stringify(logData)}`;
      console.error(errorMsg);
      throw new Error(errorMsg);
    }
